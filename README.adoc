...
Annotate the consumer method with @FluxBridgeListener Annotation, specify the topic.
The method parameter needs to be DataSpec.
Once the producer publish a message with corresponding topic, metadata and data from that message can be obt
[source, Java] 
----
@Controller
@Slf4j
